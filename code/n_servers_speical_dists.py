...
def get_setup():
    ...
    num_server = np.random.choice(num_servers).item()
    ser_key = np.random.choice(np.arange(len(ser_dics.keys()))).item()
    arrive_key = np.random.choice(np.arange(len(arrival_dics.keys()))).item()

    return (rho, num_server, arrive_key, ser_key)
...
for sample in range(5000):
    ...
    rho, num_server, arrive_key, ser_key = get_setup()

    moms_arrive, arrive_times = get_moms_realizations(rho, num_server, arrival_dics[arrive_key], sample_size, True)
    moms_ser, ser_times = get_moms_realizations(rho, num_server, ser_dics[ser_key], sample_size, False)
    moms_arrive = np.array(moms_arrive)
    moms_ser = np.array(moms_ser)
    ...
    try:
        for trails in tqdm(range(1)):
            ...
            n_Queue_single_station = N_Queue_single_station(sim_time, num_stations, ser_times, arrive_times,
                                                            num_server)
            ...
            inp_steady_0 = np.concatenate((np.log(moms_arrive), np.log(moms_ser), np.array([num_server])))
            ...
            print(1-outputs1[0][0],outputs1[0][1], rho, num_server, mom_1_ser, mom_1_ser/num_server, outputs1[0][1]/2 + rho)
        ...
        file_name =  ('rho_' + str(rho)[:5] + '_num_servers_' + str(num_server) + '_arrival_dist_' + arrival_dics[arrive_key]
                      +  '_ser_dist_' + ser_dics[ser_key] +   '_sim_time_' + str(sim_time) + 'steady_' + str(
            model_num) + '.pkl')
